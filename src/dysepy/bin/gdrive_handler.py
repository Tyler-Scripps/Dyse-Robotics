...
class GD_Handler:
	...
	def __init__(self):
		self.config = os.path.join(os.getenv('DYSE_ROOT'), 'config', 'prog', 'gdrive.yaml')
	...
	def uploadFolder(self, batch=None, folder='data'):
		...
		path = os.path.join(os.getenv('DYSE_ROOT'), folder)
	...
	def downloadBatch(self, batch=None):
		...
		for file in file_list:
			...
			file.GetContentFile(os.path.join(os.getenv('DYSE_ROOT'), 'data', file['title']))
			file = None